...
namespace NHOM1_DA1_QUANLINHANSU.GUI.UC
{
    public partial class UC_QLNhanVien : UserControl
    {
        ...
        public UC_QLNhanVien()
        {
            ...
            dataGridView_QLNV.Dock = DockStyle.Fill;
            dataGridView_QLNV.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.ColumnHeader;
        ...
        private void button_QLNV_Them_Click(object sender, EventArgs e)
        {
            ...
            var nv = new Nhanvien
            {
                ...
                GioiTinh = comboBox_QLNV_GT.Text.Trim(),
                ...
                HinhAnh = hinhAnhByte,
                SoTienPhuCap = float.Parse(textBox_QLNV_TienPC.Text.Trim()),
                TrangThai = comboBox_QLNV_TT.Text.Trim(),
                IdtaiKhoan = int.Parse(comboBox_IDTaiKhoan.SelectedValue.ToString().Trim()),
        ...
        {
            if (string.IsNullOrEmpty(textBox_QLNV_ID.Text))
            {
            ...
            {
                Idnv = int.Parse(textBox_QLNV_ID.Text.Trim()),
                TenNv = textBox_QLNV_TenNV.Text.Trim(),
                GioiTinh = comboBox_QLNV_GT.Text.Trim(),
                NgaySinh = DateOnly.FromDateTime(dateTimePicker_QLNV_NgaySinh.Value),
                Sdt = textBox_QLNV_SDT.Text.Trim(),
        ...
        private void button_QLNV_Xoa_Click(object sender, EventArgs e)
        {
            ...
            else if (MessageBox.Show("Bạn có chắc chắn muốn xóa nhân viên này không?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int idnv = int.Parse(textBox_QLNV_ID.Text.Trim());
                QLNV_BLL.XoaNhanVien(idnv);
                LoadNV();
            }
        }
        ...
        private void button_QLNV_XuatFile_Click(object sender, EventArgs e)
        {
        ...
        {
            string tuKhoa = textBox_QLNV_TimKiem.Text.Trim();
        ...
        private void dataGridView_QLNV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                ...
                textBox_QLNV_ID.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["Idnv"].Value.ToString().Trim();
                textBox_QLNV_TenNV.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["TenNv"].Value.ToString().Trim();
                comboBox_QLNV_GT.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["GioiTinh"].Value.ToString();
                dateTimePicker_QLNV_NgaySinh.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["NgaySinh"].Value.ToString().Trim();
                textBox_QLNV_SDT.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["Sdt"].Value.ToString().Trim();
                textBox_QLNV_CCCD.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["Cccd"].Value.ToString().Trim();
                textBox_QLNV_DiaChi.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["DiaChi"].Value.ToString().Trim();
                textBox_QLNV_TienPC.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["SoTienPhuCap"].Value.ToString().Trim();
                comboBox_QLNV_TT.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["TrangThai"].Value.ToString().Trim();
                comboBox_QLNV_PB.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["TenPhongBan"].Value.ToString().Trim();
                comboBox_QLNV_TD.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["TenTrinhDo"].Value.ToString().Trim();
                comboBox_QLNV_CV.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["TenCongViec"].Value.ToString().Trim();
                comboBox_IDTaiKhoan.Text = dataGridView_QLNV.Rows[e.RowIndex].Cells["TenTaiKhoan"].Value.ToString().Trim();
            }
        }
    }
}
